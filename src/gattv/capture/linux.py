...
class LinuxCaptureSource:
    ...
    def units(self) -> Iterator[CapturedUnit]:
        try:
            self._container = av.open(
                f"/dev/video{self._camera.index}",
                format="v4l2",
                options={
                    "input_format": "mjpeg",
    ...
    def detection_image(self, unit: CapturedUnit) -> np.ndarray:
        frames = self._decoder.decode(av.Packet(unit.payload))
        if not frames:
            raise CameraError("Could not decode an MJPEG frame for motion detection.")
        return frames[0].to_ndarray(format="gray")
